...
while True:
    ...
    if write_dt >= write_interval:
        ...
        #print(p1)
        if p1 >= thm and p1 < thh: tmm += 1
        elif p1 >= thm and  p1 >= thh: tmh += 1
        elif p1 < thl: tml += 1
        else:
            ver = (tmm,tmh,tml)
            if (tmm > 0 or tmh > 0) and tmm > tmh: print("J",ver)
            if (tmm > 0 or tmh > 0) and tmh > tmm: print("U",ver)
            if tml > 0: print("M",ver)
            tmh = tmm = tml = 0
